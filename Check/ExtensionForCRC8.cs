...
//            Unknown,
        ...
        /// <returns></returns>
        /*******************************************/
        public static byte CRC8(this IEnumerable<byte> data, CRC8Type type = CRC8Type.Basic)
        {
            byte result = 0;

            if (type == CRC8Type.Basic) result = data.ComputeNormalCRC8();
            else if (type == CRC8Type.Maxim) result = data.ComputeMaximCRC8();
//            else if (type == CRC8Type.Unknown) result = data.ComputeTestCRC8();
        ...
        /// <returns></returns>
        /*******************************************/
        public static IEnumerable<byte> WithCRC8(this IEnumerable<byte> data, CRC8Type type)
            => data.Append(data.CRC8(type));


        private static byte ComputeNormalCRC8(this IEnumerable<byte> data)
        {
            if (data.Count() == 0) return 0;

            byte crc = 0;
            foreach (var dt in data)
                crc = _crc8NormalTable[(crc ^ dt) & 0xff];

            return crc;
        }

        private static byte ComputeMaximCRC8(this IEnumerable<byte> data)
        {
            if (data.Count() == 0) return 0;

            byte crc = 0;
            foreach (var dt in data)
                crc = _crc8MaximTable[(crc ^ dt) & 0xff];
        ...
        private static byte[] _crc8NormalTable = 
        {
